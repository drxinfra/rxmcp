...
- `rx_complete_assignment`: выполнить задание или принять работы (результат подбирается по типу задания: Complete, Accepted, ForRework).
- `rx_create_simple_task`: создать и отправить простую задачу (срок обязателен, так требует RX).
...
Пошаговая инструкция для обычного пользователя, с получением cookie из браузера и настройкой Claude Desktop: [docs/claude-desktop.md](docs/claude-desktop.md).
...
**Cookie из браузера** (`cookie`): путь без администратора, работает при любом способе входа в RX. Войдите в RX в браузере, в панели разработчика (F12 → Application → Cookies) скопируйте значение cookie `sungero_client` и передайте `RXMCP_COOKIE="sungero_client=<значение>"`. Работает, пока жива сессия браузера. Подробно в [docs/claude-desktop.md](docs/claude-desktop.md).
